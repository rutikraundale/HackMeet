			...
			<div className="max-w-4xl mx-auto">
				<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
					<div>
						...
						<p className="text-sm md:text-base text-gray-400">Stay updated with your latest activities</p>
					</div>
					{unreadCount > 0 && (
						<div className="bg-blue-600/20 text-blue-400 border border-blue-500/30 px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-medium">
							{unreadCount} New
						</div>
					)}
				</div>
				...
				{notifications.length === 0 ? (
					<div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 md:p-12 text-center">
						<div className="bg-slate-800 w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
							<Bell className="w-8 h-8 md:w-10 md:h-10 text-gray-500 opacity-50" />
						</div>
						<h3 className="text-lg md:text-xl font-semibold mb-2">No notifications yet</h3>
					...
					<div className="space-y-3 md:space-y-4">
						{notifications.map((n) => (
							<div
								...
								className={`group relative bg-slate-900 border transition-all duration-300 rounded-xl md:rounded-2xl p-4 md:p-5 flex gap-3 md:gap-5 items-start cursor-pointer ${
									!n.isRead 
							...
							>
								...
								<div className="relative shrink-0">
									...
									/>
									{!n.isRead && (
										<div className="absolute -top-0.5 -right-0.5 w-3 h-3 md:w-4 md:h-4 bg-blue-500 rounded-full border-2 border-slate-900 animate-pulse"></div>
									)}
								</div>
								...
								<div className="flex-1 min-w-0">
									<div className="flex justify-between items-start mb-1">
										<p className="text-sm md:text-base text-gray-100 leading-relaxed">
											{n.type === "hackathon" ? (
												<span className="font-bold text-blue-400">New Hackathon</span>
											) : (
									...
									<div className="flex flex-wrap items-center gap-3 md:gap-4 text-[10px] md:text-xs text-gray-500 mt-2">
								...
								{/* Actions */}
								<div className="flex flex-col gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity duration-200 shrink-0">
									{!n.isRead && (
										<button
											onClick={(e) => {
												e.stopPropagation();
